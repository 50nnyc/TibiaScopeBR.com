...
function switchTab(tab) {
  ...
  if (tab === "world") {
    el.searchLabel.textContent = "Busca opcional";
    el.searchInput.placeholder = "Deixe vazio ou filtre pelo nome";
    el.searchInput.disabled = false;
    el.worldInput.disabled = false;
    el.notice.textContent = "Selecione um mundo e, se quiser, uma vocacao. O ranking abre a ficha completa ao clicar no personagem.";
  }
...
async function handleSearch(event) {
  ...
  try {
    if (state.tab === "character") {
      ...
      const world = el.worldInput.value.trim();
      if (!world) throw new Error("Selecione um mundo para montar o ranking.");
      const data = await fetchJson(`${API_BASE}/highscores/${encodeURIComponent(world)}/experience/all/1`);
      renderWorldRanking(data, query);
      syncUrl({ mode: "world", world, vocation: el.vocationFilter.value, query });
    }
  } catch (error) {
    renderError(error);
  }
...
function renderCharacterSkills(character) {
  const columns = getRelevantSkillColumns(character.vocation);
  const values = extractSkillValues(character.raw);
  return `
    <table class="member-table">
      <thead><tr>${columns.map((column) => `<th>${escapeHtml(column.label)}</th>`).join("")}</tr></thead>
      <tbody>
        <tr>${columns.map((column) => `<td>${escapeHtml(String(values[column.key] ?? "-"))}</td>`).join("")}</tr>
      </tbody>
    </table>
  `;
}
...
function renderGuild(data) {
  ...
  el.results.innerHTML = `
    <div class="table-toolbar">
      ...
      <strong>${filtered.length} membros filtrados</strong>
    </div>
    ${renderEntityTabs("guild", "overview")}
    <div id="detailPane">
      ${renderGuildOverview(state.currentGuild)}
...
function renderWorldRanking(data, nameFilter = "") {
  const entries = data?.highscores?.highscore_list || data?.highscores?.entries || [];
  const filtered = entries
    .map(normalizeRankingEntry)
    .filter((entry) => applyRankingFilters(entry, nameFilter))
    .sort((a, b) => b.level - a.level || b.experience - a.experience);
...
function applyRankingFilters(entry, nameFilter) {
  const vocation = el.vocationFilter.value;
  const level = el.levelFilter.value;
  const nameOk = !nameFilter || entry.name.toLowerCase().includes(nameFilter.toLowerCase());
  const vocationOk = vocation === "all" || entry.vocation.toLowerCase().includes(vocation);
  if (level === "all") return nameOk && vocationOk;
  const [min, max] = level.split("-").map(Number);
  return nameOk && vocationOk && entry.level >= min && entry.level <= max;
}
...
function saveSnapshot(character) {
  ...
  writeStorage(key, snapshots.slice(-500));
}
...
function getRelevantSkillColumns(vocation = "") {
  ...
  if (lower.includes("paladin")) {
    return [
      { key: "distance", label: "Distance skill" },
      { key: "magicLevel", label: "Magic level skill" },
    ];
  }
  if (lower.includes("monk")) {
    return [
      { key: "melee", label: "Melee skill" },
      { key: "magicLevel", label: "Magic level skill" },
    ];
  }
  if (lower.includes("druid") || lower.includes("sorcerer")) {
    return [{ key: "magicLevel", label: "Magic level skill" }];
  }
  if (lower.includes("knight")) {
    return [
...
function extractSkillValues(raw = {}) {
  return {
    magicLevel: firstValue(raw, ["magic_level", "magiclevel", "magic"]),
    distance: firstValue(raw, ["distance", "distance_fighting", "distancefighting"]),
    melee: firstValue(raw, ["melee", "fist", "fist_fighting", "fistfighting"]),
    sword: firstValue(raw, ["sword", "sword_fighting", "swordfighting"]),
    axe: firstValue(raw, ["axe", "axe_fighting", "axefighting"]),
    club: firstValue(raw, ["club", "club_fighting", "clubfighting"]),
    shielding: firstValue(raw, ["shielding", "shield"]),
  };
}
  ...
  document.querySelectorAll(".detail-tab").forEach((button) => {
    button.addEventListener("click", () => {
      ...
      if (type === "character") {
        ...
        if (detail === "experience") pane.innerHTML = renderCharacterExperience(character);
        if (detail === "deaths") pane.innerHTML = renderCharacterDeaths(character);
        if (detail === "skills") pane.innerHTML = renderCharacterSkills(character);
      } else {
        const guild = state.currentGuild;
        if (detail === "overview") pane.innerHTML = renderGuildOverview(guild);
        if (detail === "experience") pane.innerHTML = renderGuildExperience(guild);
